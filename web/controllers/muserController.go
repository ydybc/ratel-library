...
type BookCaseMController struct {
	Ctx iris.Context
}

func (c *MainMController) PostDelmark() mvc.Result {
	//	if !c.Ctx.IsAjax() {
...
func (c *MainMController) PostMovemark() mvc.Result {
	if !c.Ctx.IsAjax() {
		return mvc.Response{Code: 404}
	}
	var Json JsonMove
...
func (c *MainMController) PostAddmark() mvc.Result {
	if !c.Ctx.IsAjax() {
		return mvc.Response{Code: 404}
	}
...
func (c *MainMController) PostRegister() mvc.Result {
	var userreg UserRegister
	userreg.Username = c.Ctx.FormValue("username")
	userreg.Email = html.EscapeString(c.Ctx.FormValue("email"))
	...
	ok, info, err := service.LookupUserOnly(userreg)
	if err != nil {
		return mvc.Response{Object: service.ReAjax(info, 0, "")}
	}
	if ok {
		return mvc.Response{Object: service.ReAjax(info, 0, "")}
	}
...
func (c *MainMController) PostLogin() mvc.Result {
	var (
...
func (c *MainMController) GetOutlogin() mvc.Result {
	c.Ctx.RemoveCookie("ratel_u")
	c.Ctx.RemoveCookie("ratel_n")
	return mvc.Response{Path: "/login"}
}
func (c *MainMController) GetLogin() mvc.Result {
	id := c.Ctx.GetCookie("ratel_u", iris.CookieDecode(sc.Decode))
	name := c.Ctx.GetCookie("ratel_n", iris.CookieDecode(sc.Decode))
	...
	return mvc.View{Name: "ziyougem/User/login.html"}
}
func (c *MainMController) GetRegister() mvc.Result {
	id := c.Ctx.GetCookie("ratel_u", iris.CookieDecode(sc.Decode))
	name := c.Ctx.GetCookie("ratel_n", iris.CookieDecode(sc.Decode))
	_, err := service.IsLoginGet(id, name)
	if err == UErr.ServiceUnavailable {
	...
	c.Ctx.ViewData("WebInfo", Web)
	c.Ctx.ViewData("res4img", res4img)
	return mvc.View{Name: "ziyougem/User/register.html"}
}
func (c *MainMController) customBookcase(sort int, class int, page int) mvc.Result {
	...
	return mvc.View{Name: "ziyougem/User/bookcase.html"}
}

//func (c *MainMController) GetBookcase() mvc.Result {
//	var (
//		pageInfo ListPages
//		pno      int = 15
//		class    int = 0
...
//func (c *MainMController) GetBookcaseBy(sort int, class int, page int) mvc.Result {
//	var (
//		pageInfo ListPages
//		pno      int = 15
//	)
...
//	pageInfo.Sort = sort
//	pageInfo.Class = class
//	result, err := repositories.QueryBookcaseByIdSortClass(repositories.DB, userinfo.Userid, sort, class, page-1, pno)
//	if err != nil {
//		return mvc.Response{Code: 503}
//	}
//	for x, y := range result {
